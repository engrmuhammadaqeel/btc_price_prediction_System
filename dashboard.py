...
def load_model_and_scaler():
    ...
    try:
        # Model aur scaler files isi folder mein hain (deployment ke liye flat structure)
        base_dir = os.path.dirname(os.path.abspath(__file__))

        rf_model = joblib.load(os.path.join(base_dir, "rf_btc_prediction_model.pkl"))

        with open(os.path.join(base_dir, "rf_scaler.pkl"), "rb") as f:
            scaler = pickle.load(f)

        # rf_config.pkl optional hai - agar exist nahi karta to None use karo
        config = None
        config_path = os.path.join(base_dir, "rf_config.pkl")
        if os.path.exists(config_path):
            with open(config_path, "rb") as f:
                config = pickle.load(f)

        return rf_model, scaler, config

    except Exception as e:
        st.error(
            f"Model load nahi ho saka.\n\nError: {e}"
        )
